...
def exampleImg():
    ...
    print('X_trainArr shape ', X_train.shape)
    return X_train
X_train = exampleImg()
# print(X_train[0].shape)
def zero_pad(X, pad):
...
#     assert(A.shape == (m, n_H, n_W, n_C))
    ...
    return A, cache
# random data --> otherwise change A_prev for input data
A_prev = np.random.randn(2, 5, 7, 4)
pool_forward(A_prev = A_prev, hparameters = hparameters, mode = "max")
(A_prev, W, b, cachehparameters) = cache 
...
# pool_backward(dA, cache, mode = "max")
